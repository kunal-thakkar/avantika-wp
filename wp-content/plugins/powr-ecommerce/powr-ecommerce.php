<?php
    ...
    Description: Sell more products, downloads, or subscriptions instantly  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.2
    ...
    */
    ...
    class Powr_Ecommerce extends WP_Widget{
      public $plugin;
      //Create the widget
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Ecommerce.
        </p>
        <p>
    ...
    function register_powr_ecommerce() {
      register_widget( 'Powr_Ecommerce' );
    }
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_ecommerce_menu' );
    function powr_ecommerce_menu() {
      add_menu_page( 'POWr Ecommerce', 'POWr Ecommerce', 'manage_options', 'powr-ecommerce-settings', 'powr_ecommerce_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_ecommerce_options() {
      if ( !current_user_can( 'manage_options' ) )  {
        wp_die( __( 'You do not have sufficient permissions to access this page.' ) );
      }
      echo '<iframe id="powr-ecommerce" src="https://www.powr.io/wp-create/ecommerce" frameborder="0" width="100%" height="600px" style="position:absolute; top:0; left:0;z-index: 100;"></iframe>';
      echo '<script>';
      echo 'var ht = window.innerHeight; if(document.getElementById("wpadminbar")){ht -= document.getElementById("wpadminbar").offsetHeight;}';
      echo 'var iframe = document.getElementById("powr-ecommerce"); iframe.style.height = ht+"px"; iframe.height = ht;';
      echo '</script>';
    }
    //Redirecting to landing page when plugin is activated
    register_activation_hook(__FILE__, 'powr_ecommerce_plugin_activate');
      add_action('admin_init', 'powr_ecommerce_plugin_redirect');

      function powr_ecommerce_plugin_activate() {
      add_option('powr_ecommerce_plugin_do_activation_redirect', true);
      }

      function powr_ecommerce_plugin_redirect() {
      if (get_option('powr_ecommerce_plugin_do_activation_redirect', false)) {
          delete_option('powr_ecommerce_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-ecommerce-settings' );
          }
       }
      }
