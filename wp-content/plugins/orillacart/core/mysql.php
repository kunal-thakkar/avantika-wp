...
//return $this->loadresult();
    ...
    public function loadassoc() {
        ...
        while ($ob = mysql_fetch_assoc($this->res)) {
            $oblist[] = $ob;
        }
    ...
    public function loadarray() {
        ...
        while ($ob = mysql_fetch_array($this->res, MYSQL_NUM)) {
    ...
    public function loadobjectlist($col = null) {
        ...
        if (!$col) {
            while ($ob = mysql_fetch_object($this->res)) {
                $oblist[] = $ob;
            }
        } else {
            while ($ob = mysql_fetch_object($this->res)) {
    ...
    public function loadResult($col = NULL) {
        ...
        if ($col) {
            ...
            return is_resource($this->res) ? mysql_result($this->res, 0, $col) : false;
        } else if ($this->numRows() > 0) {

            return is_resource($this->res) ? mysql_result($this->res, 0) : false;
        }
