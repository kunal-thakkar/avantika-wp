...
class ec_admin_products{
	...
	function duplicateproduct( $productid ){
		...
		if( $success === FALSE ){
			...
			$sql = "SELECT ec_product.product_id, ec_product.title, ec_product.activate_in_store, ec_product.model_number, ec_product.description FROM ec_product WHERE ec_product.model_number = %s";
	...
	function updateproduct( $productid, $product ){
		...
		// Update GUID
		global $wpdb;
		$wpdb->query( $wpdb->prepare( "UPDATE " . $wpdb->prefix . "posts SET " . $wpdb->prefix . "posts.guid = %s WHERE " . $wpdb->prefix . "posts.ID = %d", get_permalink( $this_post_id ), $this_post_id ) );
