...
if( !empty( $users ) ){
	...
	if( $format == 'excel' ){
		...
		foreach( $results as $result ){
			...
			$option_sql = "SELECT ec_order_option.option_value FROM ec_order_option WHERE ec_order_option.orderdetail_id = %s ORDER BY ec_order_option.option_order ASC, ec_order_option.option_to_product_id ASC, ec_order_option.order_option_id ASC";
				...
				foreach( $results as $row ){
					...
					foreach($row as $line)
					{
						...
						if ($row[13] == '0') {
							...
							$option_sql = "SELECT ec_order_option.option_value FROM ec_order_option where ec_order_option.orderdetail_id = '".$row[14]."'  ORDER BY ec_order_option.option_order ASC, ec_order_option.option_to_product_id ASC, ec_order_option.order_option_id ASC";
