...
class ec_cartpage{
	...
	function __construct( $is_affirm = false ){
		...
		if( isset( $GLOBALS['ec_cart_data']->cart_data->coupon_code ) && $GLOBALS['ec_cart_data']->cart_data->coupon_code != "" ){
			...
			$this->coupon = $GLOBALS['ec_coupons']->redeem_coupon_code( $this->coupon_code );
		}else{
			$this->coupon_code = "";
		}
	...
	private function validate_submit_order_data( ){
		...
		// Basic Validation
		if( $GLOBALS['ec_user']->billing->country == "0" || $GLOBALS['ec_user']->billing->first_name == "" || $GLOBALS['ec_user']->billing->last_name == "" || $GLOBALS['ec_user']->billing->address_line_1 == "" || $GLOBALS['ec_user']->billing->city == "" ){
	...
	private function process_login_user( ){
		...
		$user = $this->mysqli->get_user_login( $email, $password );
							...
							if( $card_result ){ //Card Submitted Successfully
								...
								if( $plan_added ){ // Plan added successfully
									...
									if( $product->is_taxable || $product->vat_rate ){
										$taxable_subtotal = 0;
										$taxable_subtotal = $product->price * $quantity;
										...
										$this->tax = new ec_tax( $product->price * $quantity, $taxable_subtotal - $sales_tax_discount->coupon_discount, $vatable_subtotal, $GLOBALS['ec_user']->shipping->state, $GLOBALS['ec_user']->shipping->country, $GLOBALS['ec_user']->taxfree, $this->shipping->get_shipping_price( ) + $this->cart->get_handling_total( ) );
										$this->order_totals = new ec_order_totals( $this->cart, $GLOBALS['ec_user'], $this->shipping, $this->tax, $this->discount );
									}
