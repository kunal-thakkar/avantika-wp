...
class ec_orderdisplay{
	...
	public function send_email_receipt(){
		...
		if( $email_send_method == "1" ){
			...
			wp_mail( stripslashes( get_option( 'ec_option_bcc_email_addresses' ) ), $GLOBALS['language']->get_text( "cart_success", "cart_payment_receipt_title" ) . " " . $this->order_id, $message, implode("\r\n", $headers), $attachments );
