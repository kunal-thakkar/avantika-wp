...
class ec_accountpage{
	...
	private function process_login( ){
		...
		if( isset( $_POST['ec_account_login_password_widget'] ) ){
			$password = md5( $_POST['ec_account_login_password_widget'] );
		}else{
	...
	private function process_update_password( ){
		...
		if( $new_password != $retype_new_password )
			...
			$success = $this->mysqli->update_password( $user_id, $current_password, $new_password );
	...
	private function send_new_password_email( $email, $new_password ){
		...
		$user = $this->mysqli->get_user_login( $email, md5( $new_password ) );
