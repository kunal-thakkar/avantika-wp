...
class ec_product{
	...
	private function setup_product( $product_data ){
		...
		if( $this->is_product_details ){
			...
			if( $this->use_optionitem_quantity_tracking ){
				$quantity_results = $this->mysqli->get_option_quantity_values( $this->product_id );
				$this->option1quantity = array( );
				foreach( $quantity_results as $result_item ){
					$this->option1quantity[$result_item->optionitem_id] = $result_item->quantity;
				}
			}
	...
	public function get_deconetwork_link( ){
		...
		$link .= "callback_param_ec_action=deconetwork_add_to_cart&oid=" . $GLOBALS['ec_cart_data']->ec_cart_id . "&callback_param_ec_product_id=" . $this->product_id;
		return $link;
