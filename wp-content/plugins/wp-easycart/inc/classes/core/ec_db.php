...
class ec_db{
	...
	public static function get_tempcart_product_quantities( $session_id ){
		...
		if( !$results ){
			$sql = "SELECT SUM(tempcart.quantity) as quantity, tempcart.product_id FROM ec_tempcart as tempcart WHERE tempcart.session_id = '%s' GROUP BY tempcart.product_id";
	...
	public static function get_quantity_data( $product_id, $optionset1, $optionset2, $optionset3, $optionset4, $optionset5 ){
		...
		$result = self::$mysqli->get_results(self::$mysqli->prepare($sql, $product_id));
		...
		for($a=0; $a<count($optionset1->optionset); $a++){
			$quantity = self::get_level_1_quantity($quantity_array, $optionset1->optionset[$a]->optionitem_id);
			$arr = array(array(), $quantity);
			
			//Option 2 Loop
			for($b=0; $b<count($optionset2->optionset); $b++){
				$quantity = self::get_level_2_quantity($quantity_array, $optionset1->optionset[$a]->optionitem_id, $optionset2->optionset[$b]->optionitem_id);
				array_push($arr[0], array(array(), $quantity));
				
				//Option 3 Loop
				for($c=0; $c<count($optionset3->optionset); $c++){
					$quantity = self::get_level_3_quantity($quantity_array, $optionset1->optionset[$a]->optionitem_id, $optionset2->optionset[$b]->optionitem_id, $optionset3->optionset[$c]->optionitem_id);
					array_push($arr[0][$b][0], array(array(), $quantity));
					
					//Option 4 Loop
					for($d=0; $d<count($optionset4->optionset); $d++){
						$quantity = self::get_level_4_quantity($quantity_array, $optionset1->optionset[$a]->optionitem_id, $optionset2->optionset[$b]->optionitem_id, $optionset3->optionset[$c]->optionitem_id, $optionset4->optionset[$d]->optionitem_id);
						array_push($arr[0][$b][0][$c][0], array(array(), $quantity));
						
						//Option 5 Loop
						for($e=0; $e<count($optionset5->optionset); $e++){
							$quantity = self::get_level_5_quantity($quantity_array, $optionset1->optionset[$a]->optionitem_id, $optionset2->optionset[$b]->optionitem_id, $optionset3->optionset[$c]->optionitem_id, $optionset4->optionset[$d]->optionitem_id, $optionset5->optionset[$e]->optionitem_id);
	...
	public static function get_manufacturers( $level, $menuid, $manufacturer_id, $category_id ){
		...
		if( !$manufacturers ){
			...
			$manufacturers = self::$mysqli->get_results( $sql );	
	...
	public static function get_groups( ){
		...
		return self::$mysqli->get_results( "SELECT ec_category.category_id, ec_category.category_name, ec_category.post_id FROM ec_category ORDER BY ec_category.category_name ASC" );	
	}
	...
	public static function remove_cart_data( $session_id ){
		...
		self::$mysqli->query( self::$mysqli->prepare( "DELETE FROM ec_tempcart_data WHERE ec_tempcart_data.session_id = %s OR ( ec_tempcart_data.tempcart_time < DATE_SUB( NOW( ), INTERVAL 7 DAY ) AND ec_tempcart_data.email = '' ) OR ec_tempcart_data.tempcart_time IS NULL", $session_id ) );
	...
	public static function get_category_items( &$level, $menu_id, $submenu_id, $subsubmenu_id ){
		...
		if( $level == 0 ){
			return self::$mysqli->get_results( $sql_level0 );
		
		}else if( $level == 1 ){
			$results = self::$mysqli->get_results( self::$mysqli->prepare( $sql_level1, $menu_id ) );
			if( count( $results ) > 0 )
				return $results;
			else{
				$level = 0;
			...
			$results = self::$mysqli->get_results( self::$mysqli->prepare( $sql_level2, $submenu_id ) );
			if( count( $results ) > 0 )
				return $results;
			else{
				$level = 1;
				$menu_id = self::$mysqli->get_var( self::$mysqli->prepare( $sql_get_menuid, $submenu_id ) );
				return self::$mysqli->get_results( self::$mysqli->prepare( $sql_level1, $menu_id ) );
			}
	...
	public static function get_response_from_order_id( $order_id ){
		return self::$mysqli->get_var( self::$mysqli->prepare( "SELECT is_error FROM ec_response WHERE order_id = '%s'", $order_id ) );
	}
	...
	public static function get_user_login( $email, $password ){
		$sql = "SELECT 
				ec_user.user_id,
				ec_user.first_name, 
				ec_user.last_name,
		...
		return self::$mysqli->get_row( self::$mysqli->prepare( $sql, $email, $password ) );
	...
	public static function update_password( $user_id, $current_password, $new_password ){
		$correct_password = self::$mysqli->get_var( self::$mysqli->prepare( "SELECT ec_user.password FROM ec_user WHERE ec_user.user_id = %d", $user_id ) );
	...
	public static function get_category_list( ){
		$sql = "SELECT ec_category.* FROM ec_category";
		return self::$mysqli->get_results( $sql );	
	}
	...
	public static function get_category_list_items( $parent_id ){
		...
		if( !$category_list ){
			if( $parent_id == -1 ){
				$category_list = self::$mysqli->get_results( "SELECT ec_category.* FROM ec_category WHERE ec_category.parent_id = 0 ORDER BY ec_category.category_name ASC" );
			}else if( $parent_id )
				$category_list = self::$mysqli->get_results( self::$mysqli->prepare( "SELECT ec_category.* FROM ec_category WHERE ec_category.parent_id = %d ORDER BY ec_category.category_name ASC", $parent_id ) );
			else
				$category_list = self::$mysqli->get_results( "SELECT ec_category.* FROM ec_category WHERE ec_category.featured_category = 1 ORDER BY ec_category.category_name ASC" );
	...
	public static function get_advanced_optionsets( $product_id ){
		...
		return self::$mysqli->get_results( self::$mysqli->prepare( $sql, $product_id ) );
	}
	...
	public static function get_advanced_optionitems( $option_id ){
		...
		return self::$mysqli->get_results( self::$mysqli->prepare( $sql, $option_id ) );
	}
	...
	public static function insert_subscription_order( $product, $user, $card, $subscription_id, $coupon_code, $order_notes, $option1_name, $option2_name, $option3_name, $option4_name, $option5_name, $option1_label, $option2_label, $option3_label, $option4_label, $option5_label, $quantity, $order_totals ){
		$sql = "INSERT INTO ec_order( user_id, user_email, user_level, orderstatus_id, sub_total, tax_total, grand_total, promo_code, billing_first_name, billing_last_name, billing_address_line_1, billing_city, billing_state, billing_country, billing_zip, billing_phone, shipping_first_name, shipping_last_name, shipping_address_line_1, shipping_city, shipping_state, shipping_country, shipping_zip, shipping_phone, payment_method, creditcard_digits, subscription_id, order_customer_notes, billing_company_name, shipping_company_name, billing_address_line_2, shipping_address_line_2, order_gateway) VALUES( %d, %s, %s, %d, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %d, %s, %s, %s, %s, %s, 'stripe')";
		self::$mysqli->query( self::$mysqli->prepare( $sql, $user->user_id, $user->email, $user->user_level, 6, ( $product->price * $quantity ) + $product->subscription_signup_fee, $order_totals->tax_total + $order_totals->vat_total + $order_totals->pst_total + $order_totals->hst_total + $order_totals->gst_total, ( $product->price * $quantity ) + $product->subscription_signup_fee + $order_totals->tax_total + $order_totals->vat_total + $order_totals->pst_total + $order_totals->hst_total + $order_totals->gst_total, $coupon_code, $user->billing->first_name, $user->billing->last_name, $user->billing->address_line_1, $user->billing->city, $user->billing->state, $user->billing->country, $user->billing->zip, $user->billing->phone, $user->shipping->first_name, $user->shipping->last_name, $user->shipping->address_line_1, $user->shipping->city, $user->shipping->state, $user->shipping->country, $user->shipping->zip, $user->shipping->phone, $card->payment_method, $card->get_last_four( ), $subscription_id, $order_notes, $user->billing->company_name, $user->shipping->company_name, $user->billing->address_line_2, $user->shipping->address_line_2 ) );
	...
	public static function get_stripe_subscription( $stripe_subscription_id ){
		if( $stripe_subscription_id == "sub_00000000000000" ){ // Test Webhook
			...
			ec_subscription.paypal_password, ec_subscription.stripe_subscription_id, ec_product.image1, ec_product.trial_period_days
	...
	public static function insert_stripe_order( $subscription, $webhook_data, $user ){
		...
		self::$mysqli->query( self::$mysqli->prepare( "INSERT INTO ec_order( 
					...
					billing_phone, payment_method, creditcard_digits, stripe_charge_id, subscription_id, order_gateway
					...
					$user->billing_phone, $user->default_card_type, $user->default_card_last4, $webhook_data->charge, $subscription->subscription_id, 'stripe' ) );
	...
	public static function get_total_cart_items_by_product_id( $product_id, $session_id ){
		return self::$mysqli->get_var( self::$mysqli->prepare( "SELECT SUM( ec_tempcart.quantity ) FROM ec_tempcart WHERE ec_tempcart.product_id = %d AND ec_tempcart.session_id = %s", $product_id, $session_id ) );
	}
